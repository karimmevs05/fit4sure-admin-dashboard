...
export default function SalesPipelineProposal() {
  ...
  return (
    <main className="flex-1 space-y-6 p-8">
      {/* Proposal banner -- makes clear what's real vs. preview-only */}
      <div className="rounded-2xl border border-[#3E6594] bg-[#EAF0F7] p-4 flex items-start gap-3">
        <Info className="h-5 w-5 text-[#2E527F] flex-shrink-0 mt-0.5" />
        <div className="text-sm text-[#2E527F]">
          <p className="font-extrabold">This is a working proposal, running only on your machine right now.</p>
          <p className="mt-1 text-[#3E6594]">
            Quick-adding a lead, deleting, and moving pipeline stages are <b>real</b> and hit production data. Lead source, account type,
            company name, the Pipeline working-set, and the Follow-Up Queue are <b>preview-only</b> -- saved in this browser until we add
            real columns and a task-creation endpoint on the backend.
          </p>
          <p className="mt-1 text-[#3E6594]">
            The Pipeline board now starts <b>empty</b> every session -- tick people on the Customers tab and click
            "Add to Pipeline" to build your working set, then "✓ Done" or "Finish Session" moves them into Tasks below.
          </p>
        </div>
      </div>
            ...
            {followUps.map((f) => (
              <div key={`f-${f.customerId}`} className="rounded-xl border border-[#E4D8C9] bg-white p-3 flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-extrabold text-[#4B2B1D]">{f.name}</p>
                  {f.customerId > 0 ? (
                    <>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-[rgba(251,247,240,0.9)] rounded-2xl border border-[#2E527F] max-w-3xl w-full my-8">
            ...
            <div className="p-6 space-y-6 overflow-y-auto max-h-[calc(90vh-100px)]">
              ...
              <div>
                <h3 className="text-lg font-extrabold text-[#4B2B1D] mb-4">🧭 Lead Source & Account <span className="text-xs font-normal text-[#755B4C]">(preview)</span></h3>
                <div className="grid grid-cols-2 gap-4">
